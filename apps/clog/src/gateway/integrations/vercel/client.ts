...
export class VercelIntegrationClient {
  ...
  async collectObservations(): Promise<RuntimeObservation[]> {
    ...
    try {
      ...
      const failed = recent.filter((d) => d.state === "ERROR");
      ...
      if (failed.length > 0) {
        observations.push({
          ...
          details: failed.map((d) => `${d.name}: ${d.meta?.githubCommitMessage ?? "no message"}`).join("; "),
          severity: "warning",
          detectedAt: Date.now(),
        });
      }
  ...
  async getLogs(deploymentId: string, limit = 100, level?: string): Promise<VercelLogEntry[]> {
    ...
    const response = await fetch(
      `${this.baseUrl}/deployments/${deploymentId}/logs?${params}`,
      { headers: { Authorization: `Bearer ${this.config.token}` } }
    );
