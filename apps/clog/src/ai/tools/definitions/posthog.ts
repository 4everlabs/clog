...
const buildPostHogAssetSummaryData = async (
  ...
  try {
    insightList = normalizePostHogInsightList(await services.posthog.callMcpTool("insights-list", {
      limit: insightLimit,
      offset: 0,
    }));
  } catch {
...
const buildPostHogReleaseSummaryData = async (
  ...
  if (input.includeLogAttributes) {
    try {
      logAttributes = normalizePostHogLogAttributes(await services.posthog.callMcpTool("logs-attributes-list", {}));
    } catch {
...
const basePosthogTools = [
  ...
  },
  {
    ...
    async execute(services, input) {
      ...
      const result = await services.posthog.callMcpTool("insights-list", {
        limit: input.limit,
        offset: input.offset,
        search: input.search,
        ...input.arguments,
  ...
  },
  {
    ...
    async execute(services, input) {
      ...
      const result = await services.posthog.callMcpTool("logs-attributes-list", {
        ...input.arguments,
      });
  ...
  },
  {
    ...
    async execute(services, input) {
      ...
      const result = await services.posthog.callMcpTool("query-logs", {
        query: input.query,
        limit: input.limit,
        level: input.level,
