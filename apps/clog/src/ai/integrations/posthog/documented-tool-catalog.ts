import type { AgentToolName } from "../../tools/schema/tools";
import { POSTHOG_DOCUMENTED_TO_LIVE_TOOL_ALIASES } from "./mcp-tooling";
...
export const getPostHogDocumentedToolCatalog = (input: {
  ...
  const enrichedFeatures: PostHogDocumentedFeatureCatalog[] = filteredFeatures.map((feature) => {
    const accessibleToolNames = unique(feature.tools.flatMap((tool) => {
      ...
      return (POSTHOG_DOCUMENTED_TO_LIVE_TOOL_ALIASES[tool.name] ?? []).filter((alias) => liveToolNames.has(alias));
    }));
    ...
    const missingToolNames = feature.tools
      .filter((tool) => !liveToolNames.has(tool.name) && !(POSTHOG_DOCUMENTED_TO_LIVE_TOOL_ALIASES[tool.name] ?? []).some((alias) => liveToolNames.has(alias)))
      .map((tool) => tool.name);
