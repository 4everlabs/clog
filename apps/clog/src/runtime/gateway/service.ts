...
import type { NotionToolServices, PostHogToolServices } from "../../ai/tools/types";
import type { PostHogIntegrationClient } from "../../ai/integrations/posthog/client";
import { ShellExecutor } from "../../ai/tools/shell-executor";
import type { RuntimeStore } from "../../storage/chat";
import { isValidIanaTimezone } from "../config/settings";
import {
  normalizeRuntimeWakeupConfig,
  resolveRuntimeWakeupStoragePath,
} from "../config/wakeup";
import type { AgentGatewaySurface } from "./contracts";
import type { MonitoringLoop } from "../monitor-loop";
import type { MonitoringTickResult } from "../monitor-loop";
...
const readWakeupConfig = (env: AgentEnvironment): RuntimeWakeupConfig | null => {
  try {
    return normalizeRuntimeWakeupConfig(JSON.parse(readFileSync(resolveRuntimeWakeupStoragePath(env.storage.readOnlyDir), "utf-8")) as unknown);
  } catch {
    return null;
  }
};
...
export class AgentGateway implements AgentGatewaySurface {
  ...
  async runWakeupPass(input: RunWakeupPassInput): Promise<RunWakeupPassResult> {
    const thread = this.deps.store.seedOperatorThread("system", input.title?.trim() || "Wakeup");
  ...
  async updateWakeupConfig(input: SurfaceUpdateWakeupRequest): Promise<SurfaceUpdateWakeupResponse> {
    ...
    writeJsonAtomically(resolveRuntimeWakeupStoragePath(this.deps.env.storage.readOnlyDir), wakeup);
    return { wakeup };
  }
