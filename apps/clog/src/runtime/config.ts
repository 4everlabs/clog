...
import type { ToolSummary } from "../ai/tools/schema/tools";
import { RuntimeToolsConfigSchema, type RuntimeToolsConfig } from "../ai/tools/schema/tools";
import {
  DEFAULT_CLOG_MODEL,
...
import { summarizeAdvertisedTools } from "../ai/tools/registry";
...
export const loadAgentEnvironment = (env: NodeJS.ProcessEnv = getRuntimeProcessEnv()): AgentEnvironment => {
  ...
  const capabilities: IntegrationCapabilitySnapshot = {
    ...
    shell: {
      ...
      safeRoots: [storage.workspaceDir],
    },
  };
